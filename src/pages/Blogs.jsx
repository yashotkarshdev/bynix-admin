...
//       resetForm();
//       setErrors(prev => ({ ...prev, success: "Blog published successfully ✅" }));
//       fetchBlogs();
//     } catch (error) {
...
//       // ✅ FIRST: check response (normal errors)
//       if (error.response) {
//         if (error.response.status === 413) {
//           message = "Image too large. Max 1MB allowed.";
...
//       // ✅ SECOND: handle cases where response is missing
//       else if (error.request) {

//         // ⚠️ detect 413 manually (important)
//         if (error.message.includes("413")) {
//           message = "Image too large. Please compress it.";
//         } else if (error.message.includes("Network Error")) {
//           message = "Network issue or server not reachable";
//         } else {
...
//           )}
//           {errors.api && (
//             <div className="bg-red-100 text-red-700 p-3 rounded-xl font-semibold">
//               {errors.api}
//             </div>
//           )}
//         </div>
        ...
        <div className="xl:col-span-3 order-2 xl:order-1">
          <form onSubmit={handleSubmit} className="bg-white border border-gray-100 rounded-[2rem] p-6 md:p-8 shadow-sm space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700 ml-1">Content</label>
              <div className="bg-gray-50 rounded-2xl overflow-hidden border border-gray-200">
                <RichTextEditor theme="snow" value={content} onChange={setContent} className="bg-white min-h-[250px]" />
              </div>
              {errors.content && <p className="text-red-500 text-xs font-semibold ml-1">{errors.content}</p>}
            </div>
